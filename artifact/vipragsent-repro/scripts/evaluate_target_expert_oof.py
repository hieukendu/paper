...
def main():
 p=argparse.ArgumentParser();p.add_argument("--label",required=True);p.add_argument("--predictions",type=Path,required=True);p.add_argument("--name",required=True);p.add_argument("--candidate-dir",type=Path,default=ROOT/"answer/final_best_tuned_candidates");p.add_argument("--dev",type=Path,default=ROOT/"data/processed/vipragsent_dev.jsonl");a=p.parse_args()
 ...
 out=a.candidate_dir/"next_cycle"/"oof";out.mkdir(parents=True,exist_ok=True);(out/f"{a.name}.json").write_text(json.dumps(result,indent=2)+"\n")
