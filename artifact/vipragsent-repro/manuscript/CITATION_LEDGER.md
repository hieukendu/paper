...
| Sailor-7B source/model card | `dou-etal-2024-sailor` | 4 | PARTIAL | Primary paper and exact configured ID `sail/Sailor-7B` are verified. The repo now pins an upstream snapshot for future retrieval, but the historical result run did not record its base-model revision. |
| Vistral-7B source/model card | `nguyen-etal-2023-vistral` | 4 | PARTIAL | Official exact model card is verified and a future-retrieval snapshot is now pinned. No archival paper/DOI is supplied, and the historical result run did not record its base-model revision. |
...
| `dou-etal-2024-sailor` | Primary peer-reviewed system-demonstration paper; official model card corroborates ID | https://aclanthology.org/2024.emnlp-demo.45/ | `10.18653/v1/2024.emnlp-demo.45` | 4 | Sailor model family and exact configured identifier `sail/Sailor-7B`; snapshot `b8b49a0f02073e58db2e42e5811955dfe87ca970` is pinned only for future retrieval. | PARTIAL |
| `nguyen-etal-2023-vistral` | Official model card | https://huggingface.co/Viet-Mistral/Vistral-7B-Chat | N/A (no DOI supplied) | 4 | Exact configured model identifier `Viet-Mistral/Vistral-7B-Chat`; snapshot `d331b64e61b935cc43c2b3010ae9fb4fde599b45` is pinned only for future retrieval. | PARTIAL |
...
| --- | --- | --- | --- | --- |
| Sailor-7B QLoRA | VERIFIED_AT_PINNED_ARCHIVE_REVISION | Pinned archive revision; PEFT configuration; manifests for seeds 20260520/21/22; three adapter LFS SHA-256 identifiers. | Base-model commit and `transformers` version were not recorded in the archived historical runs. | The adapter weights and per-seed run records are traceable; do not claim exact historical base-model snapshot reproducibility. |
| Vistral-7B QLoRA | VERIFIED_AT_PINNED_ARCHIVE_REVISION | Pinned archive revision; PEFT configuration; manifests for seeds 20260520/21/22; three adapter LFS SHA-256 identifiers. | Base-model commit and `transformers` version were not recorded in the archived historical runs. | The adapter weights and per-seed run records are traceable; do not claim exact historical base-model snapshot reproducibility. |

## Provenance and anonymity notes

- The repository now pins Sailor/Vistral snapshots for future retrieval. The three Hugging Face archives independently verify the historical adapters and run manifests, while the citation/reproducibility status remains PARTIAL only for the unrecorded historical base-model revisions and software versions.
